...
public class CustomViewAbove extends ViewGroup {
    ...
    private int MARIGIN_SCROLL_ALLOWED;
    ...
    public int getDpFromDeviceInfo(Context context, int dp) {
    ...
    public CustomViewAbove(Context context, AttributeSet attrs) {
        super(context, attrs);
        MARIGIN_SCROLL_ALLOWED = getDpFromDeviceInfo(context,60);
        Log.v(TAG,""+MARIGIN_SCROLL_ALLOWED);
        initCustomViewAbove();
    }
    ...
    void initCustomViewAbove() {
        ...
        mTouchSlop = (float)(ViewConfigurationCompat.getScaledPagingTouchSlop(configuration) * 0.5);
        //factor 0.5 comes from experience, so the slide still works with inner scrollview
    ...
    private boolean isInIgnoredView(MotionEvent ev) {
        ...
        for (View v : mIgnoredViews) {
            //v.getHitRect(rect); //bullshit!!! getHitRect always refers on parent should refer global
            v.getGlobalVisibleRect(rect);
            if (rect.contains((int) ev.getRawX(), (int) ev.getRawY())) return true; //x and y should be raw!
        }
        return false;
    }
    ...
    private boolean thisSlideAllowed(float dx) {
        ...
        if (isMenuOpen()) {
            ...
            allowed = mViewBehind.menuClosedSlideAllowed(dx)
                    && mLastMotionX < MARIGIN_SCROLL_ALLOWED;
            //Only allow slide back to menu if the scroll gesture is happening on the left side
        }
        if (DEBUG)
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE:
                ...
                if(pointerIndex == -1) //sometimes pointerIndex is -1 and forces app to crash!
                    break;
                //Log.d(TAG,"PointerId: "+ pointerIndex);
                ...
                if (DEBUG) Log.v(TAG, "xDiff: " + xDiff);
